module.exports = function ({ app, dbConn }) {
  app.post("/notifications/create", (req, res) => {
    ...
    if (!notificationImage || !notificationMessage || !userId) {
      res
        .status(200)
        .jsonp({ message: "No se ha podido crear la notificación" });
    }
    const createNotification = [
      [notificationImage, notificationMessage, userId],
    ];
    const createNotificationSql =
      "INSERT INTO user_notification (notification_image, notification_message, user_id) VALUES ?";
    dbConn.query(
      createNotificationSql,
      [createNotification],
      function (error, insertedNotification) {
  ...
  app.get("/notifications/:id", (req, res) => {
    ...
    const getNotificationsSql =
      "SELECT * FROM user_notification WHERE user_id = ? ORDER BY id DESC";
    dbConn.query(
      getNotificationsSql,
      [userId],
      function (error, notifications) {
        if (notifications) {
          ...
          res
            .status(200)
            .jsonp({ message: "No se han podido cargar sus notificaciones" });
        }
      }
    );
  });
