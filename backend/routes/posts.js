module.exports = function ({ app, dbConn, upload }) {
  app.post("/posts", upload.single("post_image"), (req, res) => {
    ...
    if (!file) {
      ...
      if (postCreatedBy) {
        ...
        const createPostSql =
          "INSERT INTO post (post_content, post_category, post_created_date, post_created_by) VALUES ?";
        dbConn.query(
          createPostSql,
          [createdPost],
        ...
        res
          .status(200)
          .jsonp({ message: "No se ha podido crear esta publicación" });
      }
    }
  });
  ...
  app.get("/posts", (req, res) => {
    ...
    dbConn.query(getPostsSql, function (error, posts) {
      if (posts) {
        ...
        res
          .status(200)
          .jsonp({ message: "No se ha podido cargar sus publicaciones" });
      }
    });
  });
  ...
  app.get("/posts/:id", (req, res) => {
    ...
    const getPostSql =
      "SELECT post.id, post_content, post_category, post_created_date, post_created_by, post_number_of_reactions, user_account.user_avatar, user_account.user_full_name, user_account.user_number_of_followers FROM post INNER JOIN user_account ON post.post_created_by = user_account.id WHERE post.id = ?";
    if (!id) {
      res.status(200).jsonp({
        message: "No se han podido cargar los detalles de esta publicación ",
      });
    }
    dbConn.query(getPostSql, [id], function (error, response) {
      if (response && response.length) {
        ...
        res.status(200).jsonp({ message: "No encontrado" });
      }
    });
  });
  ...
  app.post("/posts/reactions", (req, res) => {
    ...
    dbConn.query(
      ...
      function (err, updatedPost) {
        if (err) {
          res.status(200).jsonp({ message: "Error del sistema" });
        } else if (updatedPost) {
  ...
  app.post("/posts/categories", (req, res) => {
    ...
    if (!userId || !postCategory) {
      res
        .status(200)
        .jsonp({ message: "No se han podido cargar las publicaciones" });
    }
    const getPostsSql =
      "SELECT * FROM post WHERE post_created_by = ? AND post_category = ? ORDER BY post_created_date DESC";
