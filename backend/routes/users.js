      ...
      if (email && password && fullname) {
        ...
        dbConn.query(findAccountByEmail, [email], function (error, account) {
          if (account && account.length !== 0) {
            res.status(200).jsonp({ message: "El email ya está en uso" });
          } else {
            const users = [[id, email, password, fullname, avatar]];
            const registerUserSql =
              "INSERT INTO user_account (id, user_email, user_password, user_full_name, user_avatar) VALUES ?";
            dbConn.query(
              registerUserSql,
              [users],
              function (error, insertedUser) {
                if (insertedUser) {
                  res
                    .status(200)
                    .jsonp({ avatar, insertId: insertedUser.insertId });
                } else {
                  res
                    .status(200)
                    .jsonp({ message: "No se ha podido crear la cuenta" });
                }
              }
            );
          }
        ...
        return res
          .status(200)
          .jsonp({ message: "Por favor completa los campos requeridos" });
      }
    }
  });
  ...
  app.post("/users/followers", (req, res) => {
    ...
    dbConn.query(
      ...
      function (err, updatedUser) {
        if (err) {
          res.status(200).jsonp({ message: "Error en el sistema" });
        } else if (updatedUser) {
