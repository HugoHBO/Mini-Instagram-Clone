...
const Detail = (props) => {
  ...
  const { user, selectedPost, setSelectedPost } = useContext(Context);
  ...
  const toggleFollow = async () => {
    try {
      if (post.hasFollowed) {
        ...
        const customMessage = {
          message: `${user.user_full_name} te ha seguido`,
          type: "notification",
  ...
  const toggleReaction = async () => {
    try {
      if (post.hasLiked) {
        ...
        const customMessage = {
          message: `A ${user.user_full_name} le gusta tu publicación`,
          type: "notification",
          receiverId: post.post_created_by,
        };
        await createNotification(customMessage.message);
  ...
  const createDate = () => {
    ...
    let date = post.post_created_date;
      ...
      <div className="post-detail__content">
        ...
        <div className="post-detail__main">
          <div className="post-detail__main-left">
            <img
              src={`http://localhost:8080${post?.post_content}`}
              alt={`${post?.post_created_by} - ${post?.post_created_date}`}
            />
          </div>
          <div className="post-detail__main-right">
            <div className="post-detail__creator">
              <img
                ...
                alt={user.avatar}
              />
              <span className="post-detail__post-creator" onClick={viewProfile}>
                {post?.user_full_name}
              </span>
              ...
              {user.id !== post?.post_created_by && (
                <span className="post-detail__follow" onClick={toggleFollow}>
                  {post?.hasFollowed ? "Siguiendo" : "Seguir"}
                </span>
              )}
              <div className="post-detail__date">{createDate()}</div>
            </div>
            ...
            <div className="post-detail__number-of-reactions">
              <span>
                {post?.post_number_of_reactions
                  ? `${post?.post_number_of_reactions} me gusta`
                  : ""}
              </span>
            </div>
          </div>
